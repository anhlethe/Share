...
class Category extends MX_Controller {
	...
	public function admincp_update($id=0){
		...
		$data = array(
			...
			'module'=>$this->module,
			'id'=>$id,
            'category' => $this->model->getParentAll(null)
		);
	...
	public function admincp_ajaxLoadContent(){
        $this->load->model('life/life_model','life');
        ...
        if(!empty($result))
            ...
            {
                $item->numarticles = $this->life->count_article_all($item->id);
            }
		$data = array(
			'result'=>$result,
    ...
    public function admincp_ajaxUpdateOrder(){
        $perm = modules::run('admincp/chk_perm',$this->session->userdata('ID_Module'),'w',1);
        if($perm=='permission-denied'){
            print '<script type="text/javascript">show_perm_denied()</script>';
            ...
            modules::run('admincp/saveLog',$this->module,$this->input->post('id'),'order','update',$this->input->post('value'),$this->input->post('value'));
            $this->db->set('order', 'order $this->input->post("value")');
            $this->db->where('id', $this->input->post('id'));
